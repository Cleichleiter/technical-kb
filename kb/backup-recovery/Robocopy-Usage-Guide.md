...
* Re-syncing datasets where only changes should be copied
...
Robocopy uses **bitmask-based exit codes**, not simple success/failure values. Exit codes from `0` to `7` typically indicate successful operations with varying conditions (for example, files copied or extra files detected).
...
* `/MT:n`
  Enables multi-threaded copying (maximum of 128 threads).
...
* `/MIR`
  Mirrors source to destination, including deletions.
...
**Always run with `/L` before using `/MIR`.**
