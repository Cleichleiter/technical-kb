...
<#
.SYNOPSIS
Consolidates a Backup Health run folder into a single JSON report and optional CSV/TXT summaries.

.DESCRIPTION
Reads Summary.json (preferred) and any per-check JSON artifacts in a run folder.
Outputs:
- BackupHealthReport.json (always)
- BackupHealthFailures.csv (optional)
- BackupHealthSummary.txt (optional)

.PARAMETER RunPath
Path to a specific run folder created by Invoke-BackupHealthCheck.ps1.
...
$jsonFiles = Get-ChildItem -LiteralPath $RunPath -Filter '*.json' -File -ErrorAction Stop |
    Where-Object { $_.Name -notin @('BackupHealthReport.json') }
...
# Derive section index if Summary.json is missing
$sectionIndex = $null
...
if ($summary -and $summary.Sections) {
    ...
    $failures = @(
        $artifacts |
        Where-Object { $_.Content -and $_.Content.Status -eq 'FAIL' } |
        ForEach-Object {
            [pscustomobject]@{
...
$outJson = Join-Path $OutputPath 'BackupHealthReport.json'
$report | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $outJson -Encoding UTF8

if ($ExportCsv) {
    $csvPath = Join-Path $OutputPath 'BackupHealthFailures.csv'
    $failures |
...
if ($ExportText) {
    $txtPath = Join-Path $OutputPath 'BackupHealthSummary.txt'

    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add("Backup Health Report") | Out-Null
...
[pscustomobject]@{
    OutputJson = $outJson
    OutputCsv  = if ($ExportCsv) { (Join-Path $OutputPath 'BackupHealthFailures.csv') } else { $null }
    OutputText = if ($ExportText) { (Join-Path $OutputPath 'BackupHealthSummary.txt') } else { $null }
}
